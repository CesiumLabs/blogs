...
export default function BlogCard({ blog, textColor = "black", bgColor = "white" }) {
    ...
    return <div className={`text-${textColor} text-left mt-3 ml-3 font-changa md:max-w-md-card lg:max-w-lg-card w-full shadow-2md`}>
        <div className="rounded-b-sm p-3 rounded-default flex flex-col justify-between" style={{
            ...
            <div className="ml-3">
               <div className="-ml-2 pl-2">
                    {(blog.tags.length ? blog.tags : ['nil']).map((x, i) => <a className={`text-sm text-blurple-default hover:text-blurple-200 uppercase font-bold ${i == 0 ? "" : "ml-2"}`} href={`/?tag=${encodeURIComponent(x)}`}>{x}</a>)}
                </div>
            ...
            {blog.author ? (
                <div className="flex ml-2">
                    ...
                    <p className="opacity-80 leading-none flex">
                        <a className="font-bold hover:underline hover:opacity-full" href={`/member/${blog.author.id}`}>{blog.author.username}</a>・{moment(blog.createdAt).fromNow()}
                    </p>
                </div>
            ) : null}
        </div>
